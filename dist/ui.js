import { PERSISTENCE_KEYS } from "./definitions.js";
...
export async function initUI() {
    ...
    ui = {
        ...
        touchAccelerate: document.getElementById("touch-accelerate"),
        touchBrake: document.getElementById("touch-brake"),
        touchTurbo: document.getElementById("touch-turbo"),
    };
...
export function showGameOver(scoreSystem) {
    if (!ui)
        return;
...
export function updateHUD(scoreSystem, speed, turboCharge) {
    if (!ui)
        return;
...
export async function saveHighScoreIfNeeded(score) {
    const existing = await persistence.getItem(PERSISTENCE_KEYS.HIGH_SCORE);
    const prev = existing ? parseFloat(existing) : 0;
    if (score > prev) {
        await persistence.setItem(PERSISTENCE_KEYS.HIGH_SCORE, Math.floor(score).toString());
        return score;
    }
    return prev;
}
export function animateHUDPop(element, duration = 0.2, scale = 1.2) {
...
export function flashMessage(text, duration = 0.8) {
    if (!ui)
        return;
