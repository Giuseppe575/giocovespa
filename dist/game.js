...
import { GAME_CONFIG, PERSISTENCE_KEYS, } from "./definitions.js";
import { clamp, lerp, lerpVec3, now, shakeValue, } from "./utils.js";
import { createBuildings, createCamera, createRenderer, createRoad, createScene, createStreetLights, createVespaWithRider, spawnObstacle, } from "./entities.js";
import { animateHUDPop, flashMessage, getUI, hideGameOver, hideMenu, initUI, saveHighScoreIfNeeded, showGameOver, showMenu, updateHUD, } from "./ui.js";
...
function steerPlayer(normalizedX, lerpFactor) {
...
function updateEngineSound(speed) {
    if (!audio.context || !audio.engineNode || !audio.engineGain)
        return;
    if (audio.muted) {
...
function addEventListeners() {
    ...
    const steerFromTouch = (touch, lerpFactor, evt) => {
        if (!touch)
            return;
...
function setupTouchControls(ui) {
    if (!ui.touchControls)
        return;
    const touchControlsEl = ui.touchControls;
    if (touchControlsEl.dataset.initialized === "1")
        return;
    ...
    const bindTouchButton = (element, onPress, onRelease) => {
        let active = false;
        const press = () => {
            if (active || !coarseQuery.matches)
                return;
            active = true;
    ...
    if (ui.touchAccelerate) {
        releases.push(bindTouchButton(ui.touchAccelerate, () => {
            input.up = true;
        }, () => {
            input.up = false;
        }));
    ...
    if (ui.touchBrake) {
        releases.push(bindTouchButton(ui.touchBrake, () => {
            input.down = true;
        }, () => {
            input.down = false;
        }));
    ...
    if (ui.touchTurbo) {
        releases.push(bindTouchButton(ui.touchTurbo, () => {
            input.turbo = true;
            if (gameState === "MENU") {
                startRun();
            }
...
function updatePlayer(dt) {
    ...
    if (input.turbo && !p.turboActive && p.turboCharge >= 0.999) {
        ...
        p.turboCharge = 0;
        p.targetSpeed = clamp(p.targetSpeed + GAME_CONFIG.turboBoost, p.minSpeed, p.maxSpeed + GAME_CONFIG.turboBoost);
        cameraShakeIntensity = 0.6;
        playWhoosh();
        flashMessage("TURBO!", 0.5);
        const ui = getUI();
        if (ui)
            animateHUDPop(ui.score);
    }
    if (p.turboActive) {
        turboTimeLeft -= dt;
        if (turboTimeLeft <= 0) {
        ...
        // recharge
        p.turboCharge = clamp(p.turboCharge + GAME_CONFIG.turboRechargeRate * dt, 0, 1);
    }
    ...
    if (input.right)
        lateral += 1;
    const maxX = ((GAME_CONFIG.lanes - 1) / 2) * GAME_CONFIG.laneWidth + 0.4;
    ...
    const ui = getUI();
    if (ui)
        updateHUD(scoreSystem, p.speed, p.turboCharge);
}
function updateObstacles(dt) {
    ...
    for (let i = world.obstacles.length - 1; i >= 0; i--) {
        ...
        if (!o.passed && o.mesh.position.z > p.mesh.position.z) {
            ...
            const ui = getUI();
            if (ui)
                animateHUDPop(ui.streak, 0.16, 1.15);
        }
    }
}
...
function updateCamera(dt) {
    ...
    const speedFactor = clamp((p.speed - GAME_CONFIG.baseSpeed) /
        (GAME_CONFIG.maxSpeed + GAME_CONFIG.turboBoost - GAME_CONFIG.baseSpeed), 0, 1);
    // dynamic camera distance
    baseOffset.z = 8.2 + speedFactor * 3.5;
    baseOffset.y = 3.4 + speedFactor * 0.9;
    const targetPos = new THREE.Vector3(p.mesh.position.x * 0.35, p.mesh.position.y + baseOffset.y, p.mesh.position.z + baseOffset.z);
    // camera shake
    if (cameraShakeIntensity > 0) {
        const t = now() * 8;
        targetPos.x += shakeValue(t, 0.25 * cameraShakeIntensity);
        targetPos.y += shakeValue(t + 10, 0.18 * cameraShakeIntensity);
        cameraShakeIntensity = Math.max(0, cameraShakeIntensity - dt * 2.3);
    }
    lerpVec3(cam.position, targetPos, 5 * dt);
    const lookAt = new THREE.Vector3(p.mesh.position.x, p.mesh.position.y + 1.5, p.mesh.position.z - 6);
    cam.lookAt(lookAt);
}
